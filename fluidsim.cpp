...
void FluidSim::add_particle(const Vec3f &pos) { particles.push_back(pos); }
...
void FluidSim::compute_phi() {
  ...
  for (unsigned int p = 0; p < particles.size(); ++p) {
    ...
    for (int k = max(0, cell_ind[2] - 1); k <= min(cell_ind[2] + 1, nk - 1);
         ++k) {
      for (int j = max(0, cell_ind[1] - 1); j <= min(cell_ind[1] + 1, nj - 1);
           ++j) {
        for (int i = max(0, cell_ind[0] - 1); i <= min(cell_ind[0] + 1, ni - 1);
             ++i) {
  ...
  // extend phi slightly into solids (this is a simple, naive approach, but
...
void FluidSim::compute_weights() {
  ...
  for (int k = 0; k < nk; ++k)
    for (int j = 0; j < nj; ++j)
      for (int i = 0; i < ni + 1; ++i) {
        u_weights(i, j, k) =
            1 - fraction_inside(nodal_solid_phi(i, j, k),
  ...
  for (int k = 0; k < u.nk; ++k)
    for (int j = 0; j < u.nj; ++j)
      for (int i = 1; i < u.ni - 1; ++i) {
        ...
        if (u_weights(i, j, k) > 0 &&
            (liquid_phi(i, j, k) < 0 || liquid_phi(i - 1, j, k) < 0)) {
          float theta = 1;
  ...
  for (int k = 0; k < v.nk; ++k)
    for (int j = 1; j < v.nj - 1; ++j)
      for (int i = 0; i < v.ni; ++i) {
        int index = i + j * ni + k * ni * nj;
        if (v_weights(i, j, k) > 0 &&
            (liquid_phi(i, j, k) < 0 || liquid_phi(i, j - 1, k) < 0)) {
          float theta = 1;
  ...
  for (int k = 0; k < w.nk; ++k)
    for (int j = 0; j < w.nj; ++j)
      for (int i = 1; i < w.ni - 1; ++i) {
        int index = i + j * ni + k * ni * nj;
        if (w_weights(i, j, k) > 0 &&
            (liquid_phi(i, j, k) < 0 || liquid_phi(i, j, k - 1) < 0)) {
          float theta = 1;
          ...
          if (theta < 0.01f)
            theta = 0.01f;
          w(i, j, k) -= dt *
                        (float)(pressure[index] - pressure[index - ni * nj]) /
                        dx / theta;
          w_valid(i, j, k) = 1;
        }
      }
...
// Apply several iterations of a very simple propagation of valid velocity data
// in all directions
void extrapolate(Array3f &grid, Array3c &valid) {
